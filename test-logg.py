...
#Initial class    
    swap_u = swap_usage()
    util_swap_status_u = str(stamp_now)  + " " + fqdn_srv  + " .:SECTION SWAP:. " + swap_u.util_swap_status()
    with open (file_log_name, 'a') as f: f.write (util_swap_status_u + '\n')

    util_swap_collect_u = str(stamp_now)  + " " + fqdn_srv  + " .:COLLECT INFO SWAP:. " +swap_u.util_swap_collect()
    with open (file_log_name, 'a') as f: f.write (util_swap_collect_u + '\n')

#Initial class
    cpu_u = cpu_util()
    util_cpu_status_u = str(stamp_now) + " " + fqdn_srv  + " .:SECTION CPU:. " + str(cpu_u.util_cpu_status())
    with open (file_log_name, 'a') as f: f.write (util_cpu_status_u + '\n')

    util_cpu_collect_u = str(stamp_now) + " " + fqdn_srv  + " .:COLLECT INFO CPU:. " + str(cpu_u.util_cpu_collect())
    with open (file_log_name, 'a') as f: f.write (util_cpu_collect_u + '\n')

#initial class
    disk_u = disk_util()	
    util_disk_status_u = str(stamp_now) + " " + fqdn_srv  + " .:SECTION DISKS:. " + disk_u.util_disk_status()
    with open (file_log_name, 'a') as f: f.write (util_disk_status_u + '\n')

    util_disk_collect_u = str(stamp_now) + " " + fqdn_srv  + " .:COLLECT INFO DISK:. " + disk_u.util_disk_collect()
